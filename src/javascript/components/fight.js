  ...
  return new Promise((resolve) => {
    ...
    function onKeyDown(event) {
      ...
      if (!repeat) {
        ...
        if (controls.PlayerOneCriticalHitCombination.includes(code) && !leftFighter.hasBlock && leftFighter.canCriticalHit) {
          leftFighter.criticalCombo.add(code);
          if (leftFighter.criticalCombo.size === 3) {
            putCriticalDamage(rightFighter, leftFighter);
          }
        }
        if (controls.PlayerTwoCriticalHitCombination.includes(code) && !rightFighter.hasBlock && leftFighter.canCriticalHit) {
